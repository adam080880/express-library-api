...
router.post('/', needAuth, adminOnly, (req, res, next) => {
  const upload = bookUpload().single('image')
  upload(req, res, (err) => {
    if (err) {
      res.status(400).send(response(false, req.body, 'Failed passed validation file (file type just for image and max file size is 1024KB)'))
    } else {
      next()
    }
  })
}, bookController.post)
router.patch('/:id', needAuth, adminOnly, (req, res, next) => {
  const upload = bookUpload().single('image')
  upload(req, res, (err) => {
    if (err) {
      res.status(400).send(response(false, req.body, 'Failed passed validation file (file type just for image and max file size is 1024KB)'))
