...
module.exports = {
  ...
  patch: async (req, res) => {
    ...
    if (!isFilled({ id, name, description })) return res.send(response(false, data, 'ID, Name, and description must be filled'))
    ...
    else return res.send(response(false, data, 'Server Error or Error not handled'))
  },
